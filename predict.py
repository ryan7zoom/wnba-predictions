...
def get_player_props(team_id, opponent_team_id=None, season=SEASON):
...
def build_report():
    ...
    for g in games:
        ...
        home_props = get_player_props(home_id, opponent_team_id=away_id)
        away_props = get_player_props(away_id, opponent_team_id=home_id)
                ...
                if vs_opp:
                    if vs_opp["games"]:
                        ...
                        for g_entry in vs_opp["games"]:
                            date_str = (g_entry.get("date") or "")[:10]
