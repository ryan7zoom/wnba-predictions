    ...
    for g in report:
        ...
        for side_label, players in ((g["away_team"], g["away_players"]), (g["home_team"], g["home_players"])):
            ...
            for p in players:
                ...
                block.append(f'<p class="player-name">{p["name"]} <span class="games-sampled">last {p["games_sampled"]} games</span></p>')
                ...
                for stat_key, floors in p["floors"].items():
                    for t, prob in floors.items():
                        ...
                        block.append(f'<span class="pill {tier}">{t}+ {stat_key[:3].upper()} &middot; {pct:.0f}%</span>')
                block.append('</div>')
                ...
                if vs_opp:
                    if vs_opp["games"]:
                        ...
                        for g_entry in vs_opp["games"]:
                            ...
                            lines.append(f"{date_str}: {pts_s}p/{reb_s}r/{ast_s}a")
...
.matchup-header {{ padding: 16px 18px 10px; }}
.court-line {{
  height: 3px;
  width: 100%;
...
.rest-line {{ color: var(--text-dim); font-size: 0.82em; margin: 6px 0 0; }}

.flag-stack {{ padding: 0 18px; display: flex; flex-direction: column; gap: 6px; margin-bottom: 14px; }}
.flag-chip {{
  color: var(--amber);
...
.flag-chip-inline {{ margin: 8px 0 0; }}
...
.spread-block {{ padding: 0 18px; display: flex; flex-direction: column; gap: 10px; }}
.spread-row-group {{
...
.pbar-row {{ display: flex; align-items: center; gap: 8px; margin: 4px 0; }}
.pbar-label {{ width: 34px; font-size: 0.78em; color: var(--text-dim); font-weight: 600; flex-shrink: 0; }}
.pbar-track {{ flex: 1; height: 8px; background: rgba(255,255,255,0.06); border-radius: 4px; overflow: hidden; }}
.pbar-fill {{ height: 100%; border-radius: 4px; }}
.pbar-na {{ width: 0%; }}
.pbar-value {{ width: 38px; text-align: right; font-size: 0.82em; font-weight: 700; flex-shrink: 0; }}
...
.player-block {{ padding: 6px 18px 4px; }}
.player-name {{ font-size: 0.95em; font-weight: 700; margin: 6px 0 6px; }}
.games-sampled {{ color: var(--text-dim); font-weight: 400; font-size: 0.85em; }}
.pill-row {{ display: flex; flex-wrap: wrap; gap: 7px; margin-bottom: 4px; }}
.pill {{
  font-size: 0.74em;
  font-weight: 700;
  padding: 5px 10px;
...
.pill-hot {{ background: rgba(45, 212, 191, 0.16); color: #5eead4; border: 1px solid rgba(45, 212, 191, 0.4); }}
.pill-warm {{ background: rgba(232, 99, 10, 0.16); color: #ffab6b; border: 1px solid rgba(232, 99, 10, 0.4); }}
.pill-cool {{ background: rgba(255,255,255,0.03); color: #5b6579; border: 1px solid rgba(255,255,255,0.06); }}

.vs-opp-line {{ color: var(--text-dim); font-size: 0.78em; margin: 8px 0 0; line-height: 1.5; }}
