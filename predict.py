...
def build_recent_defense_note(recent_defense, season_stats):
    """
...
def build_report():
    ...
    for g in games:
        ...
        home_recent_defense = build_recent_defense_note(get_team_recent_defense(home_id), home_stats)
        away_recent_defense = build_recent_defense_note(get_team_recent_defense(away_id), away_stats)
...
STAT_DISPLAY_NAMES = {"points": "PTS", "rebounds": "REB", "assists": "AST"}
        ...
        for side_label, players in ((g["away_team"], g["away_players"]), (g["home_team"], g["home_players"])):
            ...
            for p in players:
                ...
                for stat_key, floors in p["floors"].items():
                    ...
                    block.append(f'<span class="stat-group-label">{stat_key}</span>')
                ...
                if recent_def:
                    ...
                    def_txt = (f"Opponent defense (last {recent_def['games_counted']}): "
                               f"{recent_def['recent_pts_allowed_pg']:.1f} pts allowed/gm "
                               f"vs {recent_def['season_pts_allowed_pg']:.1f} season avg ({pct_txt})")
